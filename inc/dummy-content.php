...
function thumbnail_id_placeholder( $value, $object_id, $meta_key, $single ) {
    ...
    if (!isset( $meta_cache['galeria'] ) || empty( $meta_cache['galeria'] ) || !$meta_cache['galeria'][0] ) {
        $has_gallery = false;
    }
    // echo '<pre>'; print_r($meta_cache['galeria']); echo '</pre>';

    // if ( !$has_gallery && !is_admin() && 'galeria' == $meta_key ) {
